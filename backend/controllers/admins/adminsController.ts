...
// @desc: Creates or updates a grade to a student by ID.
// @method: POST
// @params: id
// @body: { courseId: number, grade: "A"|"B"|"C"|"D"|"F", year: 1|2|3 }
// @route /admins/students/:id/grades
export const upsertGradeToStudent = async (req: Request, res: Response) => {
  try {
    const validatedId = userIdSchema.safeParse(req.params.id);
    if (!validatedId.success) {
      return res.status(400).json({
    ...
    res.status(201).json({ message: "Grade added successfully", grade: updateGrade });
  } catch (error) {
    console.error("Error creating grade:", error);
    res.status(500).json({ error: "Failed to create grade." });
  }
